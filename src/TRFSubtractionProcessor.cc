/**
 * @file   TRFSubtractionProcessor.cc
 * @brief
 *
...
#include "TRFSubtractionProcessor.h"
...
using art::TRFSubtractionProcessor;

ClassImp(art::TRFSubtractionProcessor)

// Default constructor
TRFSubtractionProcessor::TRFSubtractionProcessor()
     ...
     fHitIDArray(NULL), fStartTiming(NULL)
{
   ...
   Register(fOffset("Offset","Offset of RF",142.0675*20.0));
   Register(fPeriod("Period","Period pf RF",142.0675));
}
...
void TRFSubtractionProcessor::Init(TEventCollection *col)
{
...
void TRFSubtractionProcessor::Process()
{
...
void TRFSubtractionProcessor::ProcessByTimeDiff()
{
   fOutData->Clear("C");
   ...
   for(Int_t iData=0, nData = (*fInData)->GetEntriesFast(); iData != nData; ++iData) {
      ...
      for (Int_t iStart = 0, nStart = start.size(); iStart < nStart; ++iStart) {
         ...
         if (fValidTimeMin < tdiff && tdiff <  fValidTimeMax) {
            ...
            outT->SetTiming( fmod(tdiff+fOffset,fPeriod));
         }
      }
   }
}
