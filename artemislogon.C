{
   ...
   dypath.Append(":./build/src/"); // when use original source in src
...
//   cf->Register(TCmdXsta::Instance());
   ...
   pri->SetOption("-o fit-to-page -P rcc4000a");
   cf->Register(pri);
...
//      gSystem->SetIncludePath(path);
   }
   {
	...
	if (1) {
	  ...
	  gInterpreter->ProcessLine("art::TModuleDecoderFactory::Instance()->Register(new art::TModuleDecoderSkip(24))");
	}
	if (1) {
	  gInterpreter->ProcessLine("art::TModuleDecoderFactory::Instance()->Register(new art::TModuleDecoder3377)");
	} else {
	  gInterpreter->ProcessLine("art::TModuleDecoderFactory::Instance()->Register(new art::TModuleDecoderSkip(42))");
	}
	if (1) {
	  gInterpreter->ProcessLine("art::TModuleDecoderFactory::Instance()->Register(new art::TModuleDecoderTS)");
	  
	} else {
	  gInterpreter->ProcessLine("art::TModuleDecoderFactory::Instance()->Register(new art::TModuleDecoderSkip(3))");
	}
	if (1) {
      gInterpreter->ProcessLine("art::TModuleDecoderFactory::Instance()->Register(new art::TModuleDecoderFERA)");
	} else { 
      gInterpreter->ProcessLine("art::TModuleDecoderFactory::Instance()->Register(new art::TModuleDecoderSkip(1))");
	}
	if (1) {
	  gInterpreter->ProcessLine("art::TModuleDecoderFactory::Instance()->Register(new art::TModuleDecoderFERET)");
	} else {
      gInterpreter->ProcessLine("art::TModuleDecoderFactory::Instance()->Register(new art::TModuleDecoderSkip(2))");
	}
	if (1) {
      gInterpreter->ProcessLine("art::TModuleDecoderFactory::Instance()->Register(new art::TModuleDecoderMXDC32_rcnp)");
	} else {
      gInterpreter->ProcessLine("art::TModuleDecoderFactory::Instance()->Register(new art::TModuleDecoderSkip(32))");
	}
