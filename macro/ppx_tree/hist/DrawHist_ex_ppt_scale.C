{
  ...
  c1->Divide(2,1);
  c1->cd(1);
  TFile *file0 = TFile::Open("output/calib_pid/hist_ex/ppt_carbon_carbon.nogate_raw.root");     
  sx->Rebin(bin);
  sx0->Rebin(bin);
  sx0->Sumw2(0);
  sx->SetMinimum(ymin);
  ...
  TFile *file1 = TFile::Open("output/calib_pid/hist_ex/ppt_carbon_carbon.root");
  c1->cd(2);
  sx->Rebin(bin);
  sx0->Rebin(bin);
  sx->SetMinimum(ymin);
