{
  TFile *file_mylar = TFile::Open("output/calib_pid/hist_ex/pph_mylar_oxygen.root ");
  TFile *file_carbon = TFile::Open("output/calib_pid/hist_ex/pph_carbon_oxygen.root ");
  ...
  TFile *fout = new TFile("output/calib_pid/hist_ex/pph_oxygen.root","RECREATE");
  h_mylar->Write();
  h_mylar0->Write();
  h_carbon->Write();
  h_carbon0->Write();
