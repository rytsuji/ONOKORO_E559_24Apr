...
int hist_ex_raw(std::string  reaction,std::string  target,std::string Sx){
  ...
  Double_t TMinGR = 142.5;//node["TMinGR"].as<Double_t>();
  Double_t TMaxGR = 156.5;//node["TMaxGR"].as<Double_t>();
  Double_t TMinLAS = node["TMinLAS"].as<Double_t>();
  Double_t TMaxLAS = node["TMaxLAS"].as<Double_t>();
  Double_t OmegaGR = 4.0/1000.0;//node["OmegaGR"].as<Double_t>();
  Double_t OmegaLAS = 20.0/1000.0;//node["OmegaLAS"].as<Double_t>();
  ...
  for(int i=0;i<n_file;i++){
    ...
    tree->Draw(Form("%f-pgr.fTKE-plas.fTKE-%f>>ht[%d]",Tinc,SxVal,i),
	       //( TGR && TLAS && TC)*Form("dTGRcorr(%f-pgr.fTKE-plas.fTKE,%f,%f,%f,%f,%f)",Tinc,Tinc,TMinGR,TMaxGR,TMinLAS,TMaxLAS),
	       TC && TGR,
	       "goff");
    tree->Draw(Form("%f-pgr.fTKE-plas.fTKE-%f>>ha[%d]",Tinc,SxVal,i),
	       AC && TGR,
	       //( TGR && TLAS && AC)*Form("dTGRcorr(%f-pgr.fTKE-plas.fTKE,%f,%f,%f,%f,%f)",Tinc,Tinc,TMinGR,TMaxGR,TMinLAS,TMaxLAS),
	       "goff");
  ...
  //Write to File
  //TFile *fout = new TFile(Form("output/calib_pid/hist_ex/%s_%s_%s.nogate.root",reaction.c_str(),target.c_str(),Sx.c_str()),"RECREATE");
  TFile *fout = new TFile(Form("output/calib_pid/hist_ex/%s_%s_%s.raw.root",reaction.c_str(),target.c_str(),Sx.c_str()),"RECREATE");
