...
int calc_err(std::string  reaction,std::string  target){
  ...
  double err_StatEffTotal=sqrt ( (pow(rerr_vdc_gr_mean,2.0)+pow(rerr_vdc_las_mean,2.0)+pow(rerr_mhit_gr_mean,2.0)+pow(rerr_mhit_las_mean,2.0)+pow(rerr_daq_mean,2.0))/pow(TotalBIScale,2.0) + pow(err_TotalBI,2.0));
  
  std::cout << "**** stat err (eff) **** " << err_StatEffTotal << std::endl;
