...
void pph_48Ca(void){
  ...
  //TFile *file_mylar = TFile::Open("output/ppx/pph_48Ca_mylar.root");
  //TFile *file_carbon = TFile::Open("output/ppx/pph_48Ca_carbon.root");
  TFile *file_mylar = TFile::Open("output/ppx_nogate/pph_48Ca_mylar.root");
  TFile *file_carbon = TFile::Open("output/ppx_nogate/pph_48Ca_carbon.root");

  Double_t scale=0.66348429*(48.0080/10.47)/(12.0/10.65);
  TFile *fout = new TFile("macro/ppx/output_sx/pph_48Ca_oxygen.root","RECREATE");
  ...
  int rebin=10;
  h_mylar->Rebin(rebin);
  h_mylar0->Rebin(rebin);
  h_carbon->Rebin(rebin);
  h_carbon0->Rebin(rebin);
  h_oxygen->Rebin(rebin);
  h_oxygen0->Rebin(rebin);
  ...
  TFile *file = new TFile("macro/ppx/output_sx/pph_48Ca_oxygen.root"); 



  
  gApplication->ProcessLine("zon");
  gApplication->ProcessLine("gcom Mylar(p,ph) with 48Ca setup");
