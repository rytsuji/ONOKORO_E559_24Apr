{
  ...
  //TCanvas *c1 = new TCanvas("c1","c1",1200,3000);  
  TCanvas *c1 = new TCanvas("c1","c1",1200,3000*0.2+540*NPad);
  c1->cd();
  TLatex xtitle;
  ...
  TPad *pad3 = new TPad("pad3", "pad3",
			lmargin,bmargin+dy*(double)(NPad-4),
  ...
  TPad *pad4 = new TPad("pad4", "pad4",
			lmargin,bmargin+dy*(double)(NPad-5)-xlabelmargin,
			1.00,bmargin+dy*(double)(NPad-4));

  pad4->SetTopMargin(0.0);
  pad4->SetBottomMargin(xlabelmargin/(dy+xlabelmargin));
  pad4->Draw();
  ...
  pad4->cd();
  gApplication->ProcessLine(".x macro/ppx/edit_hist/ppt_mylar.C");
  gApplication->ProcessLine(".x macro/ppx/edit_hist/ppt_mylar.C");
  
  c1->SaveAs("figs/sx_fit/ppt_oxygen.pdf");
  c1->SaveAs("figs/sx_fit/ppt_oxygen.png");
