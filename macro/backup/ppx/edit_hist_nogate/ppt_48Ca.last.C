{
  ...
  a0.Draw();
  TArrow a0_1(St,y0+0.05/5,St-0.5,y0+0.1/5,0.01,"");
  a0_1.Draw(); 
  TLatex t0;
  t0.SetTextAlign(12);
  t0.SetTextAngle(90);
  t0.DrawLatex(St-0.5,y0+0.11/5,Form("#scale[%e]{gnd (3/2^{+})}",0.75*scale));
  ...
  a1_16O.Draw();
  t_16O.DrawLatex(St_16O+E1_16O,y1_16O+0.06/5,Form("#scale[%e]{^{13}N}",0.75*scale));
  ...
  title.SetNDC(1);
  title.DrawLatex(.65,offset+0.8,Form("#scale[%e]{^{48}Ca(#it{p,pt})}",1.0*scale));
  title.DrawLatex(.65,offset+0.725,Form("#scale[%e]{#it{S_{t}}=22.559 MeV}",0.8*scale));
  title.DrawLatex(.65,.65,Form("#scale[%e]{TDX = %8.3f #pm %8.3f}",0.8*scale,tdx,tdx_err))
