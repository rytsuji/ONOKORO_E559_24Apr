{
  double scale=1;
  //
  //TFile *file_42Ca = TFile::Open("macro/ppx/output_sx/pph_42Ca_tave.root");
  TFile *file_42Ca = TFile::Open("output/ppx/pph_42Ca.tra_ave.root ");
  

  Double_t ymin=-0.0499/10;
  Double_t ymax=1.199/10;
  sx_tave->GetXaxis()->SetRange(sx_tave->FindBin(10),sx_tave->FindBin(45));  
  //sx_tave->GetXaxis()->SetRange(sx_tave->FindBin(15),sx_tave->FindBin(40));
  
  sx_tave->SetMinimum(ymin);
  sx_tave->SetMaximum(ymax);
  sx_tave->SetLabelSize(0.075*scale,"xy");
  
  sx_tave->SetStats(0);
  sx_tave0->SetStats(0);
  sx_tave->Draw();
  sx_tave0->Draw("same");
  
  Double_t Sh=20.236; // 7/2-                                                                                                                              
  Double_t E1=1517.540/1000.0; //3/2+                                                                                                                         
  Double_t E2=2358.284/1000.0; //1/2+
  Double_t E3=2503.418/1000.0; //5/2+  
  Double_t E4=2829.934/1000.0; //1/2+



  Double_t y1=0.03;
  TArrow a1(Sh+E1,y1,Sh+E1,y1+0.05/5,0.01,"<|");
  a1.Draw();
  TLatex t1;
  t1.SetTextAlign(12);
  t1.SetTextAngle(90);
  t1.DrawLatex(Sh+E1-0.5,y1+0.06/5,Form("#scale[%f]{1.51 MeV (3/2^{+})}",0.75*scale));

  Double_t y2=0.065;
  TArrow a2(Sh+E2,y2,Sh+E2,y2+0.05/5,0.01,"<|");  
  a2.Draw();
  TArrow a2_1(Sh+E2,y2+0.05/5,Sh+E2-0.5,y2+0.1/5,0.01,"");  
  a2_1.Draw();
  TLatex t2;
  t2.SetTextAlign(12);
  t2.SetTextAngle(90);
  t2.DrawLatex(Sh+E2-0.35-0.5,y2+0.11/5,Form("#scale[%f]{2.35 MeV (1/2^{+})}",0.75*scale));
  t2.DrawLatex(Sh+E2+0.35-0.5,y2+0.11/5,Form("#scale[%f]{ and ^{13}C}",0.75*scale));  

  //Double_t y3=0.25/10;
  //TArrow a3(Sh+E3,0.025,Sh+E3,0.075,0.01,"|>");
  //a3.Draw();
  //TLatex t3;
  //t3.SetTextAlign(12);
  //t3.SetTextAngle(90);
  //t3.DrawLatex(Sh+E3-0.5/5,-0.04/5,Form("#scale[%f]{5/2^{+}}",0.75*scale));

  Double_t y4=0.055;
  TArrow a4(Sh+E4,y4,Sh+E4,y4+0.05/5,0.01,"<|");    
  a4.Draw();
  TArrow a4_1(Sh+E4,y4+0.05/5,Sh+E4+0.5,y4+0.1/5,0.01,"");    
  a4_1.Draw();
  TLatex t4;
  t4.SetTextAlign(12);
  t4.SetTextAngle(90);
  t4.DrawLatex(Sh+E4+0.5,y4+0.11/5,Form("#scale[%f]{2.82 MeV (1/2^{+})}",0.75*scale));

  Double_t St_16O=22.793;
  Double_t E1_16O=3089.443/1000.;
  TLatex t_16O;
  t_16O.SetTextAlign(12);
  t_16O.SetTextAngle(90);

  Double_t y0_16O=0.18/10;
  //TArrow a0_16O(St_16O,y0_16O,St_16O,y0_16O+0.05,0.01,"<|");
  //a0_16O.Draw();
  //t_16O.DrawLatex(St_16O,y0_16O+0.06,Form("#scale[%f]{^{13}C}",0.75*scale));
  Double_t y1_16O=0.7/10;
  TArrow a1_16O(St_16O+E1_16O,y1_16O,St_16O+E1_16O,y1_16O+0.05/5,0.01,"<|");
  a1_16O.Draw();
  t_16O.DrawLatex(St_16O+E1_16O,y1_16O+0.06/5,Form("#scale[%f]{^{13}C}",0.75*scale));

  
  TLatex title;
  title.SetTextAlign(11);
  title.SetNDC(1);
  title.DrawLatex(.65,.8,Form("#scale[%f]{^{42}Ca(#it{p,p}^{3}He)}",1.0*scale));
  title.DrawLatex(.65,.7,Form("#scale[%f]{#it{S}_{^{3}He}=20.236 MeV}",0.8*scale));


  TF1 *f_42Ca = new TF1("f_40Ca","([0]/sqrt(2.*TMath::Pi()*[10]*[10]))*exp(-0.5*pow((x-[1])/[10],2))+([2]/sqrt(2.*TMath::Pi()*[10]*[10]))*exp(-0.5*pow((x-[3])/[10],2))+([4]/sqrt(2.*TMath::Pi()*[10]*[10]))*exp(-0.5*pow((x-[5])/[8],2))+([6]/sqrt(2.*TMath::Pi()*[10]*[10]))*exp(-0.5*pow((x-[7])/[10],2))+([8]/sqrt(2.*TMath::Pi()*[10]*[10]))*exp(-0.5*pow((x-[9])/[10],2))",-1000,1000);
  f_42Ca->SetParameters(0.05,Sh,
			0.2,Sh+E1,
			0.02,Sh+E2,
			0.02,Sh+E3,
			0.02,Sh+E4,		   			
			0.5);
  
  f_42Ca->FixParameter(5,Sh+E2);
  f_42Ca->FixParameter(7,Sh+E3);
  f_42Ca->FixParameter(9,Sh+E4);

  //sx_tave->Fit("f_42Ca","E","",18.0,22.2);
  

}
