...
int hist_sx(std::string  reaction,std::string  target,std::string ex){
  ...
  for(int i=0;i<run.size();i++){
    ...
    //hist phys
    TFile *file_phys= TFile::Open(Form("output/hist_phys_ex/run%s_%s.hist.root",run[i].c_str(),ex.c_str()));
  ...
  TFile *fout = new TFile(Form("output/ppx_ex/%s_%s_%s.root",reaction.c_str(),target.c_str(),ex.c_str()),"RECREATE");
  //fout->mkdir("phys"); 
  //phys->cd();
  fout->Add(sx);
  fout->Add(sx0);
