{
  ...
  t0.SetTextAlign(12);
  t0.SetTextAngle(90);
  t0.DrawLatex(St-0.5,y0+0.06/5,Form("#scale[%f]{gnd (3/2^{+})}",scale*0.75));
  ...
  t1.SetTextAlign(12);
  t1.SetTextAngle(90);
  t1.DrawLatex(St+E1,y1+0.06/5,Form("#scale[%f]{0.98 MeV (1/2^{+})}",0.75*scale));
  /*
  Double_t y2=0.07;
  ...
  */
  ...
  double tdx=f_44Ca->GetParameter(0)/((double) dx/Nbin);
  double tdx_err=f_44Ca->GetParError(0)/((double) dx/Nbin);
  
  TLatex title;
  title.SetTextAlign(11);
  title.SetNDC(1);
  title.DrawLatex(.55,.8,Form("#scale[%f]{^{44}Ca(#it{p,pt})^{41}K}",1.0*scale));
  //title.DrawLatex(.55,.725,Form("#scale[%f]{#it{S_{t}}=20.859 MeV}",0.8*scale));
  //title.DrawLatex(.55,.65,Form("#scale[%f]{TDX = %8.3f #pm %8.3f}",0.8*scale,tdx,tdx_err));

  TF1 *f_44Ca_peak = new TF1("f_44Ca_peak","gausn",f_44Ca->GetParameter(1)-2.0,f_44Ca->GetParameter(1)+2.0);
  f_44Ca_peak->SetParameters(f_44Ca->GetParameter(0),
                             f_44Ca->GetParameter(1),
                             f_44Ca->GetParameter(4));
  f_44Ca_peak->SetLineColor(2);
  f_44Ca_peak->SetLineWidth(1);
  f_44Ca_peak->SetLineStyle(2);    
