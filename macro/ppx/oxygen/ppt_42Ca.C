...
void ppt_42Ca(void){
  ...
  TFile *file_Ca = TFile::Open("macro/ppx/output_sx/ppt_42Ca.root");
  
  Double_t scale=0.66348429*(48.0080/10.47)/(12.0/10.65);
  Double_t scale_oxygen=0.45103820*(10.47/48.0080)/(10.0/41.958);

  TFile *fout = new TFile("macro/ppx/output_sx/ppt_42Ca_sub.root","RECREATE");
  ...
  TH1F* h_Ca0= (TH1F*) file_Ca->Get("sx0")->Clone();
  h_Ca0->SetName("h_Ca0");
  ...
  TH1F* h_Ca= (TH1F*) file_Ca->Get("sx")->Clone();
  h_Ca->SetName("h_Ca");
  ...
  h_oxygen0->Scale(scale_oxygen);
  h_oxygen->Scale(scale_oxygen);
  
  h_Ca0->Add(h_oxygen0,-1.0);
  h_Ca->Add(h_oxygen,-1.0);
  h_mylar0->Sumw2(0);
  h_oxygen0->Sumw2(0);
  h_carbon0->Sumw2(0);
  h_Ca0->Sumw2(0);  
  ...
  TFile *file = new TFile("macro/ppx/output_sx/ppt_42Ca_sub.root"); 
