...
int hist_sx_carbon(std::string  reaction,std::string  target){
  ...
  Double_t dE_gr=8.0;//sqrt(fMass*fMass+pow(0.3*(Double_t) fAtomicNumber*fMagneticField*fRho*(1.0+0.0175),2.0))-sqrt(fMass*fMass+pow(0.3*(Double_t) fAtomicNumber*fMagneticField*fRho*(1.0-0.0175),2.0));
  ...
  */
  ...
  sx_tc->Scale(scale_tdx_ub);
  sx_ac->Scale(scale_tdx_ub);
  ...
  TFile *fout = new TFile(Form("output/ppx/%s_%s.gated.root",reaction.c_str(),target.c_str()),"RECREATE");
  //fout->mkdir("phys"); 
  //phys->cd();
  fout->Add(sx);
  fout->Add(sx0);
