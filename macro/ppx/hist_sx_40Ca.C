...
int hist_sx_40Ca(std::string  reaction,std::string  target){
  ...
  for(int i=0;i<run.size();i++){
    ...
    //hist phys
    TFile *file_phys= TFile::Open(Form("output/hist_phys_40Ca/run%s.hist.root",run[i].c_str()));
  ...
  if(reaction == "ppt"){
    dE_gr=153.0-145.0;
    dOmega_gr=0.00230914;
    dOmega_las=0.00824776;
  }else if(reaction=="pph"){
    dE_gr=153.0-145.0;    
    dOmega_gr=0.00230914;
    dOmega_las=0.00834122;
