{
  ...
  TCut xcut="abs(gfp.fX-2.24459e+02)<10";
  TCut entry="Entry$<60000";  
  TH1F *ha=new TH1F("ha","ha",1000,-0.1,0.1);
  tree->Draw("gfp.fA>>ha",xcut && entry,"");
  TF1 *g5=new TF1("f","gaus(0)+gaus(3)+gaus(6)+gaus(9)+gaus(12)",-0.1,0.1);

  double constant[5]={100.0,100.0,100.0,100.0,100.0};
  double mean[5]={-0.023,-0.005,0.015,0.035,0.055};
