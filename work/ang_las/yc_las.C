...
int yc_las(){
  ...
  ofile  << "offset: " << par[0]-1.0/577.5 << std::endl; //sieve slit is placed at y = -1 mm (dz=577.5 mm)
  ...
  ofile  << "yxxx: " << par[18] << std::endl;
  ofile  << "yxxa: " << par[19] << std::endl;
  ofile  << "yxaa: " << par[20] << std::endl;
  ofile  << "yaaa: " << par[21] << std::endl;
  
  ofile  << "yy: " << par[22] << std::endl;
  ofile  << "yyxxx: " << par[23] << std::endl;
  ofile  << "yyxxa: " << par[24] << std::endl;
  ofile  << "yyxaa: " << par[25] << std::endl;
  ofile  << "yyaaa: " << par[26] << std::endl;

  ofile  << "yyyxxx: " << par[27] << std::endl;
  ofile  << "yyyxxa: " << par[28] << std::endl;
  ofile  << "yyyxaa: " << par[29] << std::endl;
  ofile  << "yyyaaa: " << par[30] << std::endl;      

  
  //ofile  << "bbb: " << par[22] << std::endl;  
  //ofile  << "bxx: " << par[23] << std::endl;
  //ofile  << "bxa: " << par[24] << std::endl;
  //ofile  << "baa: " << par[25] << std::endl;
  ...
  for(int i=0; i<nFile; i++){
    for(int j=0; j<x[i].size(); j++){
      ...
      val += par[18]*pow(x[i][j],3.0)*y[i][j];
      val += par[19]*pow(x[i][j],2.0)*a[i][j]*y[i][j];
      val += par[20]*pow(a[i][j],2.0)*x[i][j]*y[i][j];
      val += par[21]*pow(a[i][j],3.0)*y[i][j];

      val += par[22]*pow(y[i][j],2.0);
      val += par[23]*pow(y[i][j],2.0)*(x[i][j],3.0);
      val += par[24]*pow(y[i][j],2.0)*(x[i][j],2.0)*(a[i][j],1.0);
      val += par[25]*pow(y[i][j],2.0)*(x[i][j],1.0)*(a[i][j],2.0);
      val += par[26]*pow(y[i][j],2.0)*(x[i][j],0.0)*(a[i][j],3.0);

      val += par[27]*pow(y[i][j],3.0)*(x[i][j],3.0);
      val += par[28]*pow(y[i][j],3.0)*(x[i][j],2.0)*(a[i][j],1.0);
      val += par[29]*pow(y[i][j],3.0)*(x[i][j],1.0)*(a[i][j],2.0);
      val += par[30]*pow(y[i][j],3.0)*(x[i][j],0.0)*(a[i][j],3.0);
      ...
      //val += par[22]*pow(b[i][j],3.0);
      //val += par[23]*b[i][j]*pow(x[i][j],2.0);
      //val += par[24]*b[i][j]*x[i][j]*a[i][j];      
      //val += par[25]*b[i][j]*pow(a[i][j],2.0);
...
void chi2(Int_t &npar,Double_t *gin,Double_t &f,Double_t *par,Int_t iflag){
  ...
  for(int i=0; i<nFile; i++){
    for(int j=0; j<x[i].size(); j++){
      if(a_tgt[i][j]>-0.017316){
      //if(fabs(a_tgt[i][j])<0.015){      
      val = -b_tgt[i][j];
