{
  ...
  tree->Draw("gtgt.fX>>hx","","");
  hx->Fit("g1","","",-450,-430);
  double x=g1->GetParameter(1);
  TCut xcut=Form("abs(gtgt.fX-(%e))<10",x);
  //A Fit
  ...
  for(int i=0;i<5;i++){
    ...
    mean_a[i]=mean;
    acut[i]=Form("abs(gtgt.fA-(%e))<%e",mean,fabs(sigma));
  ...
  for(int i=0;i<5;i++){
    ...
    TString draw=Form("gtgt.fB>>hy[%d]",i);
  ...
  for(int i=0;i<4;i++){
	...
	g3->SetParameter(3*j+2,sigma_y);
      ...
      for(int j=0; j<3; j++){
	TCut ycut=Form("abs(gtgt.fB-%e)<%e",g3->GetParameter(3*j+1),g3->GetParameter(3*j+2));
	...
	TString draw_b=Form("gtgt.fB>>hb[%d][%d]",i,j+1);
	...
	g5->SetParameter(3*j+2,sigma_y);
      ...
      for(int j=0; j<5; j++){
	TCut ycut=Form("abs(gtgt.fB-%e)<%e",g5->GetParameter(3*j+1),g5->GetParameter(3*j+2));
	...
	TString draw_b=Form("gtgt.fB>>hb[%d][%d]",i,j);
  ...
  tree->Draw("gtgt.fB:gtgt.fA>>h_ay",xcut,"");
  ...
  for(int i=0;i<4;i++){
    if(i==0 || i==1){
      for(int j=0; j<3; j++){
        ofile  << x << " " << a[i][j+1] << " " << y[i][j+1] << " " << b[i][j+1] << " " << (2-i)*4.5/577.5 << " "<< (j+1-2)*12.0/577.5  <<std::endl;
	h_ay_fit->Fill(a[i][j+1],y[i][j+1]);
      }
    }else if(i==2){
      for(int j=0; j<4; j++){
	ofile << x << " " << a[i][j] << " " << y[i][j] << " " << b[i][j] << " " << (2-i)*4.5/577.5 << " "<< (j-2)*12.0/577.5  <<std::endl;
