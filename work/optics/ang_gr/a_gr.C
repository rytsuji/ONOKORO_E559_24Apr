...
int a_gr(){
  ...
  ofile  << "b: "  << par[17] << std::endl;
  ofile  << "bb: " << par[18] << std::endl;      
  ...
  */
  ...
  for(int i=0; i<nFile; i++){
    for(int j=0; j<x[i].size(); j++){
      ...
      val += par[17]*pow(b[i][j],1.0);
      val += par[18]*pow(b[i][j],2.0);      
      /*
...
void read_data(){
  ...
  std::string file[nFile]={"delta98.dat","delta99.dat","delta100.dat","delta101.dat","delta102.dat","edge.dat"};
