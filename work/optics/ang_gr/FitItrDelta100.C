{
  TFile *_file0 = TFile::Open("output/gr/run1034.root");  
  ...
  //A tgt Fit
  TH1F *ha_tgt[5]=new TH1F("ha_tgt","ha_tgt",1000,-0.1,0.1);
  tree->Draw("gtgt.fA>>ha",xcut,"");

  double const_a[5]={300,300,300,300,300};  

  for(int i=0; i<5; i++){
    g5->SetParameter(3*i,const_a[i]);
    g5->SetParameter(3*i+1,0.015);    
    g5->SetParameter(3*i+2,5.0/577.5*((double) i -2));    
  }
  ha->Fit("g5","","",-19./1000.,19./1000.);

  //Y tgt fit
  
  TCut acut[5];    
  for(int i=0;i<5;i++){
    acut[i]=Form("abs(gtgt.fA-(%e))<%e",5.0/577.5*(2-(double)i),0.5*5.0/577.5);
  }

  double const_b[5][5]={{0,300,300,300,0},
			{300,500,500,500,300},
  ...
  for(int i=0;i<5;i++){
    hb[i]=new TH1F(Form("hb[%d]",i),Form("hb[%d]",i),100,-0.1,0.1);
    TString drawb=Form("gtgt.fB>>hb[%d]",i);
    tree->Draw(drawb,acut[i],"");
  }
  ...
  for(int i=0;i<5;i++){
	...
	g3->SetParameter(3*j+1,(double)((j+1)-2)*12.0/577.5);
	g3->SetParameter(3*j+2,0.08);
      }
      
      hb[i]->Fit("g3","","",min_y[i],max_y[i]);      
	...
	TString draw_a=Form("gfp.fA>>ha_ycut[%d][%d]",i,j+1);
	...
	TH1F * hb4=new TH1F("hb4","hb4",500,-0.1,0.1);
      TH1F * hy4_bcut[3];
      TH1F * ha4_bcut[3];
      tree->Draw("gfp.fB>>hb4",xcut && acut[i],"");
      g3->SetParameters(300,-0.0013,0.0015,300,0.001,0.0015,300,0.004,0.0015);
      hb4->Fit("g3","","",-0.006,0.005);
	...
	TString draw_a=Form("gfp.fA>>ha_ycut[%d][%d]",i,j);
	tree->Draw(draw_a,ycut && xcut,"");
  ...
  ofstream ofile("work/ang_gr/dat/delta100.dat");
