...
void pph_48Ca(int target,double Q, double theta_gr, double theta_las, double BGR,TString name){
  ...
  */
  ...
  for(int i_kf=0; i_kf<N_kf+1; i_kf++){
	...
	if(abs(fmod(x_lab.Phi()*TMath::RadToDeg()+360.0,360.0)-180.0)<3.5 &&
	   abs(x_lab.Theta()*TMath::RadToDeg()-49.3)<3.4 &&
	   abs(p_lab.Theta()*TMath::RadToDeg()-50.5)<0.8 &&
	   abs(p_lab.Phi()*TMath::RadToDeg())<3.5 &&
	   abs(p_lab.Phi()*TMath::RadToDeg())<(4.0*p_lab.Theta()*TMath::RadToDeg()-197.3) &&
	   abs(p_lab.Phi()*TMath::RadToDeg())<(-5.0*p_lab.Theta()*TMath::RadToDeg()+260.0) &&
	   abs(p_lab.E()-p_lab.M()-141.0)<5.0) acc++;       	
      }
