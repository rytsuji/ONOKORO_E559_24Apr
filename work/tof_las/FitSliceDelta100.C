//run1033 delta=100%
{
  ...
  double xc=-1.07627e+00;
  TCut xcut=Form("abs(lfp.fX-(%e))<5.0",xc);
  
  ofstream ofile1("work/tof_las/dat/delta100.dat");
  ofstream ofile2("work/tof_las/fit/delta100_fit.dat");
  
  TFile *_file0 = TFile::Open("output/las/run1033.root");
  TH2F *hat=new TH2F("hat","LAS TOF vs a_fp(rad)",50,-0.2,0.2,100,380,470);
  TH2F *hat_slice=new TH2F("hat_slice","LAS TOF vs a_fp(rad)",1000,-0.2,0.2,1000,425,431);
