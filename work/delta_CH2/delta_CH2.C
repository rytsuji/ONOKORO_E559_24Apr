...
int delta_CH2(){
  ...
  min->mnparm(1, Form("p%d",1), vstart[1], step[1], -30, 30, ierflg);
  min->mnparm(2, Form("p%d",2), vstart[2], step[2], vstart[2]*0.99, vstart[2]*1.01, ierflg);
  min->mnparm(3, Form("p%d",3), vstart[3], step[3], 5, -5, ierflg);    
  ...
  for(int i=0; i<nFile; i++){
    for(int j=0; j<x_gr[i].size(); j++){
      ...
      val=par[0]-(sqrt(p_gr*p_gr+mp*mp)-mp)-(sqrt(p_las*p_las+mp*mp)-mp);
      T_gr[i].push_back(sqrt(p_gr*p_gr+mp*mp)-mp);
      T_las[i].push_back(sqrt(p_las*p_las+mp*mp)-mp);      
      sp[i].push_back(val);
    }
  }
...
void read_data(){
  ...
  Brho_gr.push_back(654.270*RhoGR);
  Brho_las.push_back(587.063*RhoLAS);
		     
  Brho_gr.push_back(654.270*RhoGR);  
  Brho_las.push_back(609.157*RhoLAS);

  Brho_gr.push_back(654.270*RhoGR);
  Brho_las.push_back(631.251*RhoLAS);
  
  Brho_gr.push_back(654.270*RhoGR);
  Brho_las.push_back(653.345*RhoLAS);
  
  Brho_gr.push_back(654.270*RhoGR);
  Brho_las.push_back(675.438*RhoLAS);
  
  std::string dir="work/delta_CH2/dat/";
